...
audit_archive_bundles() {
    local archive_app_path="$ARCHIVE_PATH/Products/Applications/Runner.app"
    ...
    if [ -f "$archive_app_path/Info.plist" ]; then
        local main_bundle_id=$(/usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" "$archive_app_path/Info.plist" 2>/dev/null || echo "NOT_FOUND")
    ...
    local frameworks_path="$archive_app_path/Frameworks"
    local plugins_path="$archive_app_path/PlugIns"
        ...
        for framework in "$frameworks_path"/*.framework; do
            if [ -d "$framework" ]; then
                ...
                if [ -f "$info_plist" ]; then
                    local framework_bundle_id=$(/usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" "$info_plist" 2>/dev/null || echo "NOT_FOUND")
                    ...
                    if [ "$framework_bundle_id" != "NOT_FOUND" ]; then
                        ...
                        if [ "$framework_bundle_id" = "$BUNDLE_ID" ]; then
                            ...
                            local unique_id="${BUNDLE_ID}.framework.${framework_name}.$(date +%s)"
    ...
    if [ -d "$plugins_path" ]; then
        ...
        for plugin in "$plugins_path"/*.appex; do
            if [ -d "$plugin" ]; then
                ...
                if [ -f "$info_plist" ]; then
                    local plugin_bundle_id=$(/usr/libexec/PlistBuddy -c "Print :CFBundleIdentifier" "$info_plist" 2>/dev/null || echo "NOT_FOUND")
                    ...
                    if [ "$plugin_bundle_id" != "NOT_FOUND" ]; then
                        ...
                        if [ "$plugin_bundle_id" = "$BUNDLE_ID" ]; then
                            ...
                            local unique_id="${BUNDLE_ID}.plugin.${plugin_name}.$(date +%s)"
                            echo "      🔧 Fixing collision: $unique_id"
...
# Create export options plist with collision prevention
...
EXPORT_METHOD="${PROFILE_TYPE:-app-store}"
case "$EXPORT_METHOD" in
    "app-store"|"ad-hoc"|"enterprise"|"development")
        echo "📤 Export method: $EXPORT_METHOD"
        ;;
    *)
        echo "⚠️ Unknown profile type: $EXPORT_METHOD, defaulting to app-store"
